...
@{@%
    std::string nuweb::blockCommentReference::fileUtf8(filePosition& l_filePosition) const{
        if(m_lastBlockComment){
            ...
            std::string returnString = indexableText::progressFilePosition(l_filePosition, "/*");
            ...
            while(wordSeparator != std::string::npos){
                ...
                if(l_filePosition.m_column > 60 || wordLength == 0)
            ...
            if(l_filePosition.m_column == 0)
                returnString += std::string(indentation, ' ') + indexableText::progressFilePosition(l_filePosition, " */");
